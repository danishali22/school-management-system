...
if ($result->num_rows == 1) {
    ...
    $parent_old_password = $row['parent_password'];
}
...
if (isset($_POST['stu_new_pass_btn'])) {
    ...
    if ($old_password  == $parent_old_password) {
        if ($new_password == $confirm_password) {
            $sql = "UPDATE `parent` SET `parent_password` = '$new_password' WHERE parent_email = '$parent_email' ";
            // echo $sql;
            // exit;
            $conn->query($sql);
?>
    ...
    if ($old_password  != $parent_old_password && $new_password != $confirm_password) {
    ?>
        <script>
            $(document).ready(function() {
                Swal.fire('Error!',
...
<div class="col-md-10 mt-2 mx-auto p-5">
    <h1 class="text-center bg-danger bg-gradient text-white my-3 my-heading">Change Your Password</h1>
    <form method="post">
        <div class="mb-3">
