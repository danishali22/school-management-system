...
if ($result->num_rows == 1) {
    ...
    $student_old_password = $row['student_password'];
}
...
if (isset($_POST['stu_new_pass_btn'])) {
    ...
    if ($old_password  == $student_old_password) {
        if ($new_password == $confirm_password) {
            $sql = "UPDATE `students` SET `student_password` = '$new_password' WHERE student_email = '$student_email' ";
            // echo $sql;
            // exit;
            $conn->query($sql);
?>
    ...
    if ($old_password  != $student_old_password && $new_password != $confirm_password) {
    ?>
        <script>
            $(document).ready(function() {
                Swal.fire('Error!',
